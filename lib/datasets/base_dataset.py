...
class BaseDataset(data.Dataset):
    ...
    def pad_image(self, image, h, w, size, padvalue):           #扩充图片
    ...
    def rand_crop(self, image, label):      #边缘裁剪
        h, w = image.shape[:-1]
    ...
    def random_brightness(self, img):           #随机亮度
    ...
    def gen_sample(self, image, label,              #创样品
    ...
    def reduce_zero_label(self, labelmap):          #减少0类别
    ...
    def inference(self, config, model, image, flip=False):          #推断
        size = image.size()
    ...
    def multi_scale_inference(self, config, model, image, scales=[1], flip=False):          #多尺度推断
