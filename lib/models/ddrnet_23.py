...
class DualResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        #return x_
        #pred=x_.data.max(1)[1]
        #pred1 = torch.argmax(x_,dim=1)
        #return x_
...
def DualResNet_imagenet(cfg, pretrained=False):
    print('pretrained ', pretrained, cfg.MODEL.PRETRAINED)
    model = DualResNet(BasicBlock, [2, 2, 2, 2], num_classes=2, planes=64, spp_planes=128, head_planes=128, augment=True)
    if pretrained:
        ...
        model_dict.update(pretrained_state)         #加载预训练模型
